...
test('pages should be set if pagination is enabled', () => {
  const wrapper = shallow(<DataSort data={data} data={data} paginate render={() => null} />)
...
test('pagination should be controlled', () => {
  const instance = shallow(<DataSort data={data} activePage={0} />).instance()
...
test('sortBy should be controlled', () => {
  const instance = shallow(<DataSort data={data} sortBy="id" />).instance()
...
test('direction should be controlled', () => {
  const instance = shallow(<DataSort data={data} direction="asc" />).instance()
...
test('search should be controlled', () => {
  const instance = shallow(<DataSort data={data} searchQuery="test" />).instance()
